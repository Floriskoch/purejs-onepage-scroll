...
function onePageScroll(element, options) {
  ...
  this.moveDown = function(el3) {
    ...
    if(!next) {
      ...
      pos = (Math.abs(pos) + next.clientHeight) * -1;
      //pos = (index * next.clientHeight) * -1;
    }
  ...
  this.moveUp = function(el4) {
    ...
    if(!next) {
      ...
      pos = (Math.abs(pos) - current.clientHeight) * -1;
      //pos = ((next.dataset.index - 1) * wh) * -1;
    }
    var next_index = next.dataset.index;
    _removeClass(current, "active");
    _addClass(next, "active");
...
// TODO: loop setting not adjusted to pixel height yet.
